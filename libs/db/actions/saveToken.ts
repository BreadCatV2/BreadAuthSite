...
export default async function saveToken(user_id:string, username:string, uuid:string, refresh_token:string, callback_url:string) {
    ...
    try {
        ...
        if (rows.length > 0) {
            await conn.query('UPDATE tokens SET refresh_token = ?, callback_url = ? WHERE user_id = ? AND uuid = ?', [refresh_token, callback_url, user_id, uuid]);
        } else {
            //if the user doesn't have a webhook registered, insert it
            await conn.query('INSERT INTO tokens (user_id, refresh_token, username, uuid, callback_url) VALUES (?, ?, ?, ?, ?)', [user_id, refresh_token, username, uuid, callback_url]);
        }
        return true;
    } catch (err) {
        console.error(err);
